...
const FormValidation = () => {
  ...
  const validateField = (name, value) => {
    ...
    if (!value  && name !== "probation_period") return "invalid"; // Field is empty
  ...
  const getData = () => {
    setLoading(true);
    if(id){
      fetch(`${process.env.REACT_APP_API_URL}/api/employee/role/detail2`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${auth.token}`,
        ...
        .then((res) => {
          if (res.status) {
            ...
            if (res.data.employee_waiting.supervisor_waiting) {
              setDefaultEmployee({
                id: res.data.employee_waiting.supervisor_waiting.id,
                label: res.data.employee_waiting.supervisor_waiting.first_name + ' ' + res.data.employee_waiting.supervisor_waiting.last_name,
                image: res.data.employee_waiting.supervisor_waiting.image
              })
            }

            setLocations(res.data.locations)
            setDepatements(res.data.depatements)
