...
interface TableProps {
  ...
  handleStepChange(newStap: number): void;
  isFetching?: boolean;
  error?: string;
  handleRetry(): void
}
...
const Table =  React.memo((props: TableProps) => {
  ...
  const total_page = props["total_page"] || 0;
  ...
  const handlePageChange = (newIndex: number) => {
    props.handlePageChange(newIndex)
  };

  const handleStepChange = (newStep: number) => {
    props.handleStepChange(newStep)
  };
  ...
  return (
    <>
      ...
      <div className="table-responsive" style={{ overflowX: "auto", WebkitOverflowScrolling: "touch"}}>
        ...
        {/***afficher le loading ou les error  */}
      ...
      {pagination && (
        <Pagination onPageChange={(newIndex) => handlePageChange(newIndex)} total_page={total_page} index={index} step={step} sizePerPageList={sizePerPageList} onStepChange={(newStep) => handleStepChange(newStep)} />
      )}
    </>
  );
});
