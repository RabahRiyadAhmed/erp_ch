...
const ImageCropper = ({ onImageCropped, defaultImage, token, aspectRatio,defaultBody }) => {
    ...
    const initCropper = () => {
        if (imageRef.current) {
            cropperInstance.current = new Cropper(imageRef.current, {
                ...
                zoomable: true,
                scalable: true,
                aspectRatio: aspectRatio || NaN, // Utilisation de l'aspectRatio ou libre si non défini
            });
        }
    };
    ...
    useEffect(() => {
        ...
        if (defaultImage) {
           console.log(defaultImage,defaultBody)
            getFile(defaultImage, token,defaultBody).then(setDFImage);
            console.log(dfImage)
        }
